...
5. أجب دائماً بالعربية الفصحى وبصيغة JSON فقط دون أي نص خارج الـ JSON.`;
  ...
  try {
    if (mode === 'general') {
      ...
      const searchUrls = [
        `https://islamqa.info/ar/search/?q=${q}`,
        `https://www.islamweb.net/ar/fatwa/search/?q=${q}`,
      ];
      let ctx = '', sourceUrl = '';
      for (const url of searchUrls) {
        try {
          const html = await fetchWithTimeout(url);
          const urls = extractArticleUrls(html, ['islamqa', 'islamweb']);
          if (urls.length > 0) {
            const aHtml = await fetchWithTimeout(urls[0]);
            const text = stripHtml(aHtml);
            if (text.length > 150) { ctx = text; sourceUrl = urls[0]; break; }
          }
        } catch (_) { continue; }
      }
      const system = `أنت أداة استخراج نصوص إسلامية فقط.
${ctx ? 'النص المستخرج من المصدر:\n' + ctx : 'لم يُعثر على نتائج في المصادر.'}
قواعد صارمة: استخرج فقط من النص المقدم. لا تضف شيئاً من عندك.
أجب بـ JSON فقط:
{"answer":"...","quranRef":"الآيات إن وُجدت في النص وإلا اتركه فارغاً","hadithRef":"الأحاديث إن وُجدت في النص وإلا اتركه فارغاً","scholarsRef":"أقوال العلماء إن وُجدت في النص وإلا اتركه فارغاً","summary":"خلاصة من النص فقط","sourceUrl":"${sourceUrl}"}`;
      const msg = await client.messages.create({
        model: 'claude-sonnet-4-5', max_tokens: 2000,
        system, messages: [{ role: 'user', content: question }]
      });
      const clean = msg.content[0].text.replace(/```json/gi, '').replace(/```/g, '').trim();
      try { return res.status(200).json(JSON.parse(clean)); }
      catch (_) { return res.status(200).json({ error: 'parse error' }); }
    }
